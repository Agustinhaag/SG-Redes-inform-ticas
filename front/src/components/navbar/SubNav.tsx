...
const SubNav: React.FC<SubNavProps> = ({ typeClass, dataUser }) => {
  ...
  const classHeader: string = `flex flex-col md:gap-6 gap-4 md:visible invisible md:flex-row text-custom-grey justify-start fixed md:relative top-0 right-0 text-right ${
    !token ? "" : "md:justify-between"
  }  md:w-1/3 w-1/2 min-w-56 z-50 md:py-0 md:px-0 px-5 pt-14 translate-x-full md:translate-x-0 md:h-6`;
